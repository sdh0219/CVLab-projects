...
DEFAULT_OUTPUT_DIR = PROJECT_DIR / "outputs" / "gradient_boosting_recovery_1500_improved_balanced"
...
def grouped_train_test_split(
    ...
    labels = set(y.unique())
    overall_distribution = y.value_counts(normalize=True).sort_index()
    target_test_rows = len(y) * test_size
    best_split = None
    ...
    for offset in range(max_attempts):
        ...
        train_labels = set(y.iloc[train_index].unique())
        test_labels = set(y.iloc[test_index].unique())
        if train_labels != labels or test_labels != labels:
            continue

        test_distribution = y.iloc[test_index].value_counts(normalize=True).sort_index()
        distribution_gap = (
            test_distribution.reindex(overall_distribution.index, fill_value=0)
            - overall_distribution
        ).abs().sum()
        size_gap = abs(len(test_index) - target_test_rows) / len(y)
        score = float(distribution_gap + size_gap)
        if score < best_score:
    ...
    if best_split is not None:
        return best_split[0], best_split[1], best_seed

    splitter = GroupShuffleSplit(n_splits=1, test_size=test_size, random_state=random_state)
    train_index, test_index = next(splitter.split(x, y, groups=groups))
    return train_index, test_index, random_state
...
def save_feature_importance_plot(importance: pd.DataFrame, output_path: Path, top_n: int) -> None:
    ...
    ax.barh(top["original_feature"], top["importance"], color="#8a5a44")
...
def train_and_evaluate(args: argparse.Namespace) -> None:
    ...
    if args.class_weight == "balanced":
        fit_params["model__sample_weight"] = compute_sample_weight(class_weight="balanced", y=y_train)
    ...
    joblib.dump(model, output_dir / "gradient_boosting_recovery_model.joblib")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a gradient boosting model for post-earthquake economic recovery cycle prediction."
    )
