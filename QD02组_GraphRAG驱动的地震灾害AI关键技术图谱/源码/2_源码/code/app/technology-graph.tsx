...
const pipelineSteps: PipelineStep[] = [
  ...
  },
  {
    ...
    title: "证据图构建",
    output: "证据图谱",
    detail: "把实体、关系、声明和原文片段连接成可追溯图谱，每条边保留来源、文本块、置信度和时间。",
...
const supplementalExportTables: ExportTable[] = [
  {
    name: "atlas.graphml",
    fields: "GraphML 图结构文件，包含节点、关系和图拓扑信息。",
  },
  {
    name: "neo4j_import.cypher",
    fields: "Cypher 导入脚本，用于 Neo4j 图数据库复现。",
  },
];
...
function pipelineDetail(step: PipelineStep): { goal: string; input: string; files: string; quality: string; status: string } {
  const details: Record<string, { goal: string; input: string; files: string; quality: string; status: string }> = {
    ...
    "02": { goal: "从地震专题语料中抽取 AI 技术、任务、数据、模型、案例、限制和政策节点，并建立证据关系。", input: "地震专题文本块 chunks.jsonl。", files: "entities.jsonl、relations.jsonl、claims.jsonl。", quality: "每条关系需要保留 doc_id、chunk_id、evidence_text、confidence 和复核状态。", status: "已完成规则抽取，大模型抽取和混合抽取接口已保留。" },
...
function exportFileType(name: string, fields: string): string {
  ...
  if (lower.endsWith(".cypher")) return "Cypher 脚本";
...
function enrichExport(table: ExportTable): ExportCard {
  ...
  const purposeByName: Record<string, string> = {
    ...
    "atlas.graphml": "保存完整图结构，适合在图分析工具中复现网络拓扑。",
    "neo4j_import.cypher": "保存 Neo4j 导入语句，用于快速复现图数据库展示。",
    "atlas_quality_report.json": "记录图谱质量检查结果、缺口和异常项，用于说明成果可靠性。",
    "community_reports.json": "保存技术社区摘要、代表实体和证据来源，用于全局检索和报告归纳。",
    "community_reports.csv": "保存技术社区摘要表，便于报告表格化展示和人工复核。",
  ...
  const core = coreExportNames.has(table.name);
...
export function TechnologyGraphExplorer() {
  ...
  const [activePipelineStepId, setActivePipelineStepId] = useState("01");
  ...
  const displayExportTables = [
    ...activeExportTables,
    ...supplementalExportTables.filter((item) => !activeExportTables.some((table) => table.name === item.name)),
  ];
  ...
  const exportCards = displayExportTables.map(enrichExport);
  const exportCategories = ["全部成果", ...Array.from(new Set(exportCards.map((item) => item.category)))];
  const categoryFilteredExports = activeExportCategory === "全部成果" ? exportCards : exportCards.filter((item) => item.category === activeExportCategory);
  const coreExports = categoryFilteredExports.filter((item) => item.core);
  const foldedExports = categoryFilteredExports.filter((item) => !item.core);
  const filteredExports = showMoreExports ? categoryFilteredExports : coreExports.length ? coreExports : categoryFilteredExports.slice(0, 8);
          ...
          <section className="panel">
            ...
            <div className="filter-chip-grid export-category-chips">
              {exportCategories.map((category) => (
                <button className={category === activeExportCategory ? "selected" : ""} key={category} onClick={() => { setActiveExportCategory(category); setActiveExportName(""); setShowMoreExports(false); }} type="button">
                  {category}
                </button>
              ))}
            </div>
            <div className="export-section-head">
              <strong>{showMoreExports ? "全部导出文件" : "核心成果文件"}</strong>
              <span>{filteredExports.length} 个文件</span>
            </div>
            ...
            {foldedExports.length ? (
              <button className="toggle-button more-export-toggle" onClick={() => { setShowMoreExports((value) => !value); setActiveExportName(""); setShowExportFields(false); }} type="button">
                {showMoreExports ? "收起更多导出文件" : `更多导出文件 ${foldedExports.length} 个`}
              </button>
            ) : null}
          </section>
