...
def evaluate(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            predictions.extend(pred.cpu().numpy()); targets.extend(y.cpu().numpy())
...
def main():
    ...
    if gaps:
        raise RuntimeError(f"数据存在 {len(gaps)} 个非小时间隔")
    ...
    model = WaterLevelLSTM(hidden_size=32).to(device)
    ...
    checkpoint = torch.load(ROOT / "lstm_model.pt", map_location=device, weights_only=True)
    model.load_state_dict(checkpoint["model_state"])
    test_loss, pred_norm, target_norm = evaluate(model, test_loader, criterion, device)
    predictions, targets = pred_norm * std + mean, target_norm * std + mean
    ...
    axes[0].plot(history["train"], label="train"); axes[0].plot(history["validation"], label="validation")
    axes[0].set(title="LSTM training loss", xlabel="Epoch", ylabel="MSE")
